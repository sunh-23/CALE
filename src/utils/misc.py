...
def validate(val_loader, model, args, device, epoch=None) -> float:
...
def get_train_transform(resizing='default', random_horizontal_flip=True, random_color_jitter=False,
                        resize_size=224, norm_mean=(0.485, 0.456, 0.406), norm_std=(0.229, 0.224, 0.225), scale=(0.5, 1.0)):
    """
    resizing mode:
...
class TransformFixMatch(object):
    def __init__(self, weak_transform, normalize_transform):
        self.weak = T.Compose([
            weak_transform,
            normalize_transform
        ])
...
def get_transforms(args):
    src_train_transform = get_train_transform(
        args.train_resizing,
    ...
    tgt_train_transform = get_train_transform(
        args.train_resizing,
        random_horizontal_flip=not args.no_hflip,
        random_color_jitter=True,
        resize_size=args.resize_size,
    ...
    src_train_transform = TransformFixMatch(src_train_transform, normalize)
    tgt_train_transform = TransformFixMatch(tgt_train_transform, normalize)
    ...
    return src_train_transform, tgt_train_transform, val_transform
